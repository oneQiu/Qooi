    ...
    <div className="auxiliary-zone">
      ...
      >
        {templateList.length !== 0 && (
          <Panel header="模板库" key="template" showArrow={false} collapsible="disabled">
            {templateList.map((i, idx) => (
              <div key={idx} className="item-warp" onClick={() => onFieldClick(i)}>
        ...
        <Panel header="字段库" key="field" showArrow={false} collapsible="disabled">
          {fieldList.map((item) => (
            <div key={item.key} className="item-warp" onClick={() => onFieldClick(item)}>
