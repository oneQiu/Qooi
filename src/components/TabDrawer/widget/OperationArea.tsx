...
export default ({ keyHandles, activeKey, hideReload, operations, moreCount }: IProps) => {
  ...
  return (
    <div className={'operation-area'}>
      ...
      {operations.slice(0, moreCount).map((i) => (
        <Button className={'color-6e'} onClick={i.onClick} key={i.key} type={'text'}>
          {i.text}
        </Button>
      ))}
          ...
          overlay={
            <Menu>
              {operations.slice(moreCount).map((i) => (
                <Menu.Item key={i.key} onClick={i.onClick} className={'drop-down-menu-item'}>
                  {i.text}
                </Menu.Item>
              ))}
            </Menu>
