 ...
 */
export default forwardRef<TDRef, TabDrawerProps>(
  (
    {
      tabs,
      hideReload = false,
    ...
    const initOpen = getUrlParams()[urlParamName] ?? defaultOpen;
    ...
    const renderItem = ({
      ...
      if (hasChild) {
        return <Menu.SubMenu {...props}>{children.map((i) => renderItem(i))}</Menu.SubMenu>;
      }
    ...
    return (
      <Drawer
        ...
        title={getTitle()}
        {...drawerProps}
        extra={<OperationArea {...{ hideReload, moreCount, operations, activeKey, keyHandles }} />}
        closable={false}
        maskStyle={{
